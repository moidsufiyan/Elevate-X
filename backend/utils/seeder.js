...
const parseVal = (str) => {
  if (!str) return 0;
  let numeric = parseFloat(str.replace(/[^0-9.]/g, ""));
...
const seedInvestors = [
  {
    ...
    description: "One of India's most prominent venture capital firms, backing exceptional founders building transformative companies across all sectors and stages.",
    sectors: ["Fintech", "E-commerce", "Enterprise Software", "Healthcare", "Consumer Tech"],
    stagePreference: ["Seed", "Series A", "Series B", "Series C+"],
    investmentRange: { min: parseVal("$500K"), max: parseVal("$50M") },
    portfolioCompanies: ["Byju's", "Zomato", "Pine Labs", "Freshworks", "Razorpay"],
    notableInvestments: [
    ...
    investmentPhilosophy: "We partner with bold entrepreneurs to build iconic companies that create lasting value. Our approach combines patient capital with deep operational expertise.",
    valueAdd: [
  ...
  },
  {
    ...
    location: "Bengaluru, India",
    description: "Early-stage venture capital firm focused on partnering with exceptional entrepreneurs building category-defining companies in India and Southeast Asia.",
    sectors: ["Fintech", "E-commerce", "Enterprise Software", "Consumer Tech", "Healthcare"],
    stagePreference: ["Seed", "Series A", "Series B"],
    investmentRange: { min: parseVal("$250K"), max: parseVal("$25M") },
    portfolioCompanies: ["Flipkart", "Swiggy", "Freshworks", "BookMyShow", "BrowserStack"],
    notableInvestments: [
    ...
    investmentPhilosophy: "We back entrepreneurs from the earliest stages and help them build enduring companies that define new categories and create significant value.",
    valueAdd: [
  ...
  },
  {
    ...
    location: "Bengaluru, India",
    description: "India-focused early-stage venture capital fund backing passionate entrepreneurs building disruptive technology companies.",
    sectors: ["Fintech", "Healthcare", "Education", "Enterprise Software", "Consumer Tech"],
    stagePreference: ["Seed", "Series A", "Series B"],
    investmentRange: { min: parseVal("$500K"), max: parseVal("$15M") },
    portfolioCompanies: ["Snapdeal", "Dream11", "Myntra", "Urban Ladder", "CureFit"],
    notableInvestments: [
    ...
    investmentPhilosophy: "We believe in backing entrepreneurs who are building transformative businesses that can create significant impact and value in the Indian market.",
    valueAdd: [
  ...
  },
  {
    ...
    description: "Early-stage venture capital fund investing in passionate entrepreneurs building technology-enabled businesses across India.",
    sectors: ["Fintech", "E-commerce", "Healthcare", "Education", "Deep Tech"],
    stagePreference: ["Pre-Seed", "Seed", "Series A"],
    investmentRange: { min: parseVal("$100K"), max: parseVal("$5M") },
    portfolioCompanies: ["Dunzo", "Unacademy", "GreyOrange", "Purplle", "Cashify"],
    notableInvestments: [
    ...
    investmentPhilosophy: "We partner with entrepreneurs at the earliest stages, providing not just capital but hands-on support to help build category-defining companies.",
    valueAdd: [
  ...
  },
  {
    ...
    location: "Mumbai, India",
    description: "Chairman Emeritus of Tata Sons and one of India's most respected angel investors, backing innovative startups across multiple sectors.",
    sectors: ["E-commerce", "Fintech", "Healthcare", "Consumer Tech", "Deep Tech"],
    stagePreference: ["Seed", "Series A"],
    investmentRange: { min: parseVal("$250K"), max: parseVal("$2M") },
    portfolioCompanies: ["Ola", "Paytm", "Snapdeal", "Urban Ladder", "CarDekho"],
    notableInvestments: [
    ...
    investmentPhilosophy: "I invest in entrepreneurs who are passionate about solving real problems and have the potential to make a meaningful impact on society.",
    valueAdd: [
  ...
  },
  {
    ...
    logo: "https://images.unsplash.com/photo-1551434678-e076c223a692?w=200&h=200&fit=crop",
    location: "Bengaluru, India",
    description: "Early-stage venture capital firm focused on investing in technology companies in India and the US, with deep expertise in enterprise software and consumer internet.",
    sectors: ["Enterprise Software", "Fintech", "Healthcare", "Consumer Tech", "Deep Tech"],
    stagePreference: ["Seed", "Series A", "Series B"],
    investmentRange: { min: parseVal("$500K"), max: parseVal("$20M") },
    portfolioCompanies: ["Postman", "Unacademy", "Turtlemint", "H2O.ai", "Delhivery"],
    notableInvestments: [
    ...
    investmentPhilosophy: "We partner with exceptional entrepreneurs building large, sustainable businesses by providing capital, expertise, and networks.",
    valueAdd: [
  ...
  },
  {
    ...
    description: "Government-backed fund of funds supporting the startup ecosystem by investing in SEBI-registered venture capital funds that invest in Indian startups.",
    sectors: ["Fintech", "E-commerce", "Manufacturing", "Agriculture", "Healthcare", "Education"],
    stagePreference: ["Seed", "Series A", "Series B"],
    investmentRange: { min: parseVal("$50M"), max: parseVal("$200M") },
    portfolioCompanies: ["Indirect through partner funds"],
    notableInvestments: [
      { company: "Multiple funds", round: "Fund Investment", amount: "$1.5B committed" }
    ],
    ...
    investmentPhilosophy: "Supporting the startup ecosystem by providing patient capital to venture funds that invest in innovative Indian startups across all sectors.",
    valueAdd: [
  ...
  },
  {
    ...
    logo: "https://images.unsplash.com/photo-1560472354-b33ff0c44a43?w=200&h=200&fit=crop",
    location: "New Delhi, India",
    description: "India's largest angel investor network with 450+ investors who have collectively invested in 750+ startups across various sectors.",
    sectors: ["Fintech", "Healthcare", "E-commerce", "Enterprise Software"],
    stagePreference: ["Pre-Seed", "Seed"],
    investmentRange: { min: parseVal("$25K"), max: parseVal("$1M") },
    ...
    investmentPhilosophy: "We believe in democratizing angel investing and supporting entrepreneurs at the earliest stages with capital, mentorship, and networks.",
    valueAdd: [
...
export const seedDatabase = async () => {
  try {
    const count = await InvestorProfile.countDocuments();
    if (count === 0) {
      console.log("[Seeder] Database is empty, seeding initial investor profiles...");
      await InvestorProfile.create(seedInvestors);
      console.log("[Seeder] Database seeded successfully with investor profiles!");
    } else {
      console.log("[Seeder] Database already contains investor profiles, skipping seeding.");
    }

    // Seed Admin User
    const adminCount = await User.countDocuments({ role: "admin" });
    if (adminCount === 0) {
      console.log("[Seeder] No admin user found, creating default admin...");
      await User.create({
        name: "Elevate Admin",
        email: "admin@elevate-x.com",
        password: "Admin@123", // Will be hashed by pre-save hook
        role: "admin",
        isVerified: true,
      });
      console.log("[Seeder] Default admin user created successfully! (admin@elevate-x.com / Admin@123)");
    }
  } catch (error) {
