...
const BlogPost = () => {
  ...
  // Find blog post by ID from dummy data
  const post = blogPosts.find((p) => p.id === id);

  // Show not found if blog post doesn't exist
  if (!post) {
    return <NotFound />;
  }
  ...
  const renderMarkdown = (markdown: string) => {
    let html = markdown
      ...
      .replace(
        /!\[(.*?)\]\((.*?)\)/g,
        '<img src="$2" alt="$1" class="my-4 rounded-lg max-w-full h-auto" />'
      )
      .replace(
        /```([\s\S]*?)```/g,
        '<pre class="bg-muted p-4 rounded-md overflow-x-auto mb-4"><code>$1</code></pre>'
      )
      .replace(
        /^\> (.*$)/gm,
