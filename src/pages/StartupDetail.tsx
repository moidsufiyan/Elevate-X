...
const StartupDetail = () => {
  ...
  const yearsActive = startup ? new Date().getFullYear() - startup.foundingYear : 0;
        ...
        <main className="pt-24 pb-16">
          <div className="container mx-auto px-6 text-center">
            ...
            <p className="text-stargaze-600 dark:text-stargaze-400 mb-8">
              The startup you're looking for doesn't exist or there was an error loading the data.
            </p>
              ...
              <div className="md:w-2/3 p-6 md:pt-10">
                ...
                <div className="flex flex-wrap items-center gap-4 text-stargaze-600 dark:text-stargaze-300 mb-6">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span>Founded {startup.foundingYear} ({yearsActive === 0 ? "New" : `${yearsActive}y`})</span>
                  </div>
          ...
          <AnimatedSection delay={100}>
            <Tabs defaultValue="about" onValueChange={setActiveTab} className="w-full max-w-5xl mx-auto">
              ...
              <TabsContent value="about" className="bg-white dark:bg-stargaze-900 p-6 rounded-xl shadow-subtle">
                <h2 className="text-2xl font-bold mb-4 text-stargaze-900 dark:text-white">About {startup.name}</h2>
              ...
              <TabsContent value="founders" className="bg-white dark:bg-stargaze-900 p-6 rounded-xl shadow-subtle">
                <h2 className="text-2xl font-bold mb-6 text-stargaze-900 dark:text-white">Founding Team</h2>
                
                {startup.founders && startup.founders.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {startup.founders.map((founder, index) => (
                      <div key={index} className="flex items-center gap-4 p-4 border border-stargaze-200 dark:border-stargaze-800 rounded-lg">
                        ...
                        <div>
                          <h3 className="font-bold text-stargaze-900 dark:text-white">{founder.name}</h3>
                          <p className="text-stargaze-600 dark:text-stargaze-400">{founder.role}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ) : (
                  <p className="text-stargaze-600 dark:text-stargaze-400">Founder information not available.</p>
                )}
              </TabsContent>
              
              <TabsContent value="details" className="bg-white dark:bg-stargaze-900 p-6 rounded-xl shadow-subtle">
                <h2 className="text-2xl font-bold mb-6 text-stargaze-900 dark:text-white">Startup Details</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Industry</h3>
                    <p className="text-stargaze-600 dark:text-stargaze-400 mb-6">{startup.industry}</p>
                    
                    <h3 className="text-lg font-semibold mb-3">Stage</h3>
                    <p className="text-stargaze-600 dark:text-stargaze-400 mb-6">{startup.stage}</p>
                    
                    <h3 className="text-lg font-semibold mb-3">Founded</h3>
                    <p className="text-stargaze-600 dark:text-stargaze-400 mb-6">{startup.foundingYear}</p>
                  </div>
                  
                  <div>
                    <h3 className="text-lg font-semibold mb-3">Location</h3>
                    <p className="text-stargaze-600 dark:text-stargaze-400 mb-6">{startup.location}</p>
                    
                    <h3 className="text-lg font-semibold mb-3">Team Size</h3>
                    <p className="text-stargaze-600 dark:text-stargaze-400 mb-6">{startup.employees} employees</p>
                    
                    <h3 className="text-lg font-semibold mb-3">Funding</h3>
                    <p className="text-stargaze-600 dark:text-stargaze-400">{startup.funding || "Not disclosed"}</p>
                  </div>
                </div>
              </TabsContent>
            </Tabs>
